...
REM %COMPILER% %INCLUDES% %OPTIONS% -Ofxwinapp.s fxwinapp.c
REM %ASSEMBLER% %ASM_OPTIONS% fxwinapp.s -o fxwinapp.obj

%LINKER% %LINK_OPT% -HB DRIVER_B2Console.obj DRIVER_FMXSDCard.obj DRIVER_B2SDCard.obj -LCL -O %DIST%\%KERNELNAME%.BIN
REM %FXLINKER% -i "%DIST%\%KERNELNAME%.BIN" -o "%DIST%\%KERNELNAME%.DLL" -v
