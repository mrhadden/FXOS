...
typedef struct {
    ...
    unsigned char number_ok_dos_fats;
    unsigned short root_dir_entries;
    unsigned short total_sectors_short; // if zero, later field is used
    unsigned char media_descriptor;
    unsigned short fat_size_sectors;
    unsigned short sectors_per_track;
    unsigned short number_ok_dos_heads;
...
#define FILE_ATTR_LONG_NAME_MASK	( FILE_ATTR_LONG_NAME | FILE_ATTR_DIRECTORY | FILE_ATTR_ARCHIVE )
...
#define FAT_PARTITION_TYPE_DOS71	(0x0C)
...
typedef VOID (*DOSINITDEVICE)(VOID);
...
typedef struct _FXDosDevice
{
	...
	DOSINITDEVICE   pfReset;
	DOSINITDEVICE   pfUninit;
...
typedef DOSHANDLE FAR *PDOSHANDLE;
...
*/
#define FXDOS_CONSOLE	0x01
#define FXDOS_KEYBOARD	0x02
#define FXDOS_MOUSE  	0x03
...
EXPORT_FUNC_DOS(FileWrite)
HRESULT k_dos_write(FIL* fp, const void* buff, UINT btw, UINT* bw);

EXPORT_FUNC_DOS(FileSeek)
HRESULT k_dos_lseek(FIL* fp, FSIZE_t ofs);

HRESULT k_dos_truncate (FIL* fp);
HRESULT k_dos_sync (FIL* fp);
EXPORT_FUNC_DOS(DirOpen)
HRESULT k_dos_opendir (DIR* dp, const TCHAR* path);

EXPORT_FUNC_DOS(DirClose)
HRESULT k_dos_closedir (DIR* dp);
HRESULT k_dos_readdir (DIR* dp, FILINFO* fno);
EXPORT_FUNC_DOS(FileFindFirst)
HRESULT k_dos_findfirst (DIR* dp, FILINFO* fno, const TCHAR* path, const TCHAR* pattern);
EXPORT_FUNC_DOS(FileFindNext)
HRESULT k_dos_findnext (DIR* dp, FILINFO* fno);
EXPORT_FUNC_DOS(DirCreate)
HRESULT k_dos_mkdir (const TCHAR* path);
EXPORT_FUNC_DOS(FileDelete)
HRESULT k_dos_unlink (const TCHAR* path);
EXPORT_FUNC_DOS(FileRename)
HRESULT k_dos_rename (const TCHAR* path_old, const TCHAR* path_new);
HRESULT k_dos_stat (const TCHAR* path, FILINFO* fno);
HRESULT k_dos_chmod (const TCHAR* path, BYTE attr, BYTE mask);
HRESULT k_dos_utime (const TCHAR* path, const FILINFO* fno);
HRESULT k_dos_chdir (const TCHAR* path);
HRESULT k_dos_chdrive (const TCHAR* path);
HRESULT k_dos_getcwd (TCHAR* buff, UINT len);
EXPORT_FUNC_DOS(FileGetFreeSpace)
HRESULT k_dos_getfree (const TCHAR* path, DWORD* nclst, FATFS** fatfs);
EXPORT_FUNC_DOS(VolumeGetLabel)
HRESULT k_dos_getlabel (const TCHAR* path, TCHAR* label, DWORD* vsn);
EXPORT_FUNC_DOS(VolumeSetLabel)
HRESULT k_dos_setlabel (const TCHAR* label);
HRESULT k_dos_forward (FIL* fp, UINT(*func)(const BYTE*,UINT), UINT btf, UINT* bf);
HRESULT k_dos_expand (FIL* fp, FSIZE_t fsz, BYTE opt);
HRESULT k_dos_mount (FATFS* fs, const TCHAR* path, BYTE opt);
HRESULT k_dos_mkfs (const TCHAR* path, const MKFS_PARM* opt, void* work, UINT len);
HRESULT k_dos_fdisk (BYTE pdrv, const LBA_t ptbl[], void* work);
HRESULT k_dos_setcp (WORD cp);
